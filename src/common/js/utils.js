...
let vue = new Vue();
// console.log($mRoutesConfig["index"])

// const test = (Vue, vm) => console.log(Vue, vm)
    ...
    return new Promise(function(resolve, reject) {
      ...
      that.$api.qiniuToken().then((res) => {
          if (res.code == 1) {
            ...
            let key = file.name;
            console.log(key)
            qiniuUploader.upload(file.path,
              (res) => {
                // 每个文件上传成功后,处理相关的事情
  ...
  rplaceRichImg(content) {
    let str = content.replace(/<img[^>]*>/gi, function(match, capture) {
      ...
      if (match.indexOf("style") != -1) {
        ...
        return match.replace(/<img/gi, '<img style="width:100%;height:auto;"');
